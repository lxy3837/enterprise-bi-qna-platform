...
it('boots the built plugin graph and renders a fixture session end to end', async () => {
  ...
  if (clientBuildValue('DSH_CLIENT_BUILD_PROFILE') === 'official') {
    ...
    // Self-branded build: the WenMark square renders the "问" glyph inside
    // its own svg, so identify it by viewBox plus that embedded text instead
    // of the removed whale path data.
    const wenMark = [...document.querySelectorAll<SVGSVGElement>('svg[viewBox="0 0 24 24"]')]
      .find(svg => svg.textContent?.includes('问'))
    expect(wenMark).not.toBeUndefined()
    // The version badge was dropped; the product name replaces "DSH Local
    // Build" in the sidebar brand seat and again as the empty-hero headline.
    expect(screen.getAllByText('Intelligent Q&A Workbench').length).toBeGreaterThan(0)
    expect(screen.queryByText(/DSH Local Build/)).toBeNull()
  }
