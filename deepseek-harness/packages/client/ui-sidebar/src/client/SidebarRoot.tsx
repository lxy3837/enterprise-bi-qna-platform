...
import {
  IconNewChatOutline16, IconPanelLeftOutline16, Tooltip, WenMark,
          ...
          >
            <span className={css.brandIdentity} aria-hidden="true">
              <span className={css.brandMark}>
                {renderSlot('sidebar.brand.mark', { size: 24 }, { fallback: <WenMark size={24} /> })}
              </span>
              <span className={css.brandName}>
                {renderSlot('sidebar.brand.name', {}, {
                  fallback: <span className={css.productBrand}>{t('brand.localBuild')}</span>,
                })}
              </span>
            </span>
          </button>
        )}
        {/* Rail resting state is the product mark; hovering swaps in the
